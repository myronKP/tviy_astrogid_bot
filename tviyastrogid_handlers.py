...
async def explanation_cards_handler(callback: CallbackQuery, state: FSMContext):
    ...
    def get_chatgpt_response():
        client = openai.OpenAI(api_key="sk-proj-nCgPrywg_gCfZbgDJivfYVgGRNC0ZZ6oOxmg4hkBx6RjxFJMXt7NtUxKwTy6Q40d7CteokcsnI"
                                       "T3BlbkFJDpLXPGZxgP6Amdu1SaMaC9F5jAvthpB4PXpqUbQE0F59GBR4gRtc15b55wNduGz0BdBpgyl0cA")
        prompt = f"""
...
Пиши українською, образно, натхненно, але зрозуміло. Обсяг — до 100 слів.
    ...
    result = await asyncio.to_thread(get_chatgpt_response)
...
- Лінія здоров’я: {health_line}"""
    ...
    from openai import OpenAI
    def get_chatgpt_response():
        client = OpenAI(api_key="sk-proj-nCgPrywg_gCfZbgDJivfYVgGRNC0ZZ6oOxmg4hkBx6RjxFJMXt7NtUxKwTy6Q40d7CteokcsnI"
                                "T3BlbkFJDpLXPGZxgP6Amdu1SaMaC9F5jAvthpB4PXpqUbQE0F59GBR4gRtc15b55wNduGz0BdBpgyl0cA")
        prompt = f"""
...
"""
    ...
    answer = await asyncio.to_thread(get_chatgpt_response)
...
async def choose_partner_sign(callback: CallbackQuery, state: FSMContext):
    ...
    from openai import OpenAI
    def get_chatgpt_response(sign1, sign2):
        client = OpenAI(api_key="sk-proj-nCgPrywg_gCfZbgDJivfYVgGRNC0ZZ6oOxmg4hkBx6RjxFJMXt7NtUxKwTy6Q40d7CteokcsnI"
                                "T3BlbkFJDpLXPGZxgP6Amdu1SaMaC9F5jAvthpB4PXpqUbQE0F59GBR4gRtc15b55wNduGz0BdBpgyl0cA")
        prompt = f"""
...
"""
    ...
    answer = await asyncio.to_thread(get_chatgpt_response, your_sign, partner_sign)
...
async def next_question(callback: CallbackQuery, state: FSMContext):
    ...
    if current_state == ArchetypeTest.q1.state:
        ...
        if person == "person_you":
            ...
            def get_chatgpt_response(you, partner):
                client = OpenAI(api_key="sk-proj-nCgPrywg_gCfZbgDJivfYVgGRNC0ZZ6oOxmg4hkBx6RjxFJMXt7NtUxKwTy6Q40d7CteokcsnI"
                                        "T3BlbkFJDpLXPGZxgP6Amdu1SaMaC9F5jAvthpB4PXpqUbQE0F59GBR4gRtc15b55wNduGz0BdBpgyl0cA")
                prompt = f"""
...
Ось архетипи: {you} і {partner}
            ...
            answer = await asyncio.to_thread(get_chatgpt_response, you, partner)
            await callback.message.answer(f"🔮 Відповідь Астрологині:\n\n{answer}")
...
async def interpret_dream(message: Message, state: FSMContext):
    ...
    def get_chatgpt_response(person_dream):
        client = OpenAI(api_key="sk-proj-nCgPrywg_gCfZbgDJivfYVgGRNC0ZZ6oOxmg4hkBx6RjxFJMXt7NtUxKwTy6Q40d7CteokcsnI"
                                "T3BlbkFJDpLXPGZxgP6Amdu1SaMaC9F5jAvthpB4PXpqUbQE0F59GBR4gRtc15b55wNduGz0BdBpgyl0cA")  # заміни на свій ключ
...
Ось сон: {person_dream}
    ...
    answer = await asyncio.to_thread(get_chatgpt_response, person_dream)
...
async def astroal_habit(message: Message, state: FSMContext):
    ...
    def get_chatgpt_response():
        client = OpenAI(api_key="sk-proj-nCgPrywg_gCfZbgDJivfYVgGRNC0ZZ6oOxmg4hkBx6RjxFJMXt7NtUxKwTy6Q40d7CteokcsnI"
                                "T3BlbkFJDpLXPGZxgP6Amdu1SaMaC9F5jAvthpB4PXpqUbQE0F59GBR4gRtc15b55wNduGz0BdBpgyl0cA")
        prompt = """
Вигадай одну унікальну астральну звичку дня (до 20 слів) Без пояснень. 
Формат:
[текст]
        """
        response = client.chat.completions.create(
...
async def interpret_problem(message: Message, state: FSMContext):
    from openai import OpenAI

    person_problem = message.text

    def get_chatgpt_response(problem: str):
        client = OpenAI(api_key="sk-proj-nCgPrywg_gCfZbgDJivfYVgGRNC0ZZ6oOxmg4hkBx6RjxFJMXt7NtUxKwTy6Q40d7CteokcsnI"
                                "T3BlbkFJDpLXPGZxgP6Amdu1SaMaC9F5jAvthpB4PXpqUbQE0F59GBR4gRtc15b55wNduGz0BdBpgyl0cA")  # 🔐 заміни на свій
        prompt = f"""
...
"""
    ...
    answer = await asyncio.to_thread(get_chatgpt_response, person_problem)
